...
def getData(file,start_time,end_time,transition=[False,0,0,0]): #transition = [bool,int motor, float trigger_level, bool rising_edge]
	pos = []
	force = []
	time = []
	bag = rosbag.Bag(file)
	...
	if transition[0]:
		...
		while(not done):
			prev = curr
			curr = pos[start_time,transition[1]]
			if transition[3]:
				if curr > transition[2] and prev < curr:
					done = True
			else:
				if curr < transition[2] and prev > curr:
					done = True
...
def getTransitions(file):
	...
	for topic, msg, t in bag.read_messages(topics=["/roboy_dep/transition"]):
		...
		tran.base = baseline[matrix_filename_to_base_index[tran.m]]
	 	...
	 	tran.data = getData(file,tran.start,tran.end,transition=[True,tran.trigger_motor,tran.trigger_level,tran.trigger_edge])
	 	trans.append(tran)
	bag.close()
	return trans
